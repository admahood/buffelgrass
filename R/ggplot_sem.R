...
ggsem_filt <- function(params, filename, title="Path Model",layout_df = NA, rename_nodes =F,
                   cols =  c("#E41A1C", "#377EB8", "grey80"), new_node_names = NA,
                   layout = "auto", alpha = 0.05, exclude = "none", h2bmask = FALSE) {
  
  # filter this part ahead of time to get the portion you want
  # params <- lavaan::standardizedSolution(fit) %>%
  #   filter(lhs != "exclude",
  #          rhs != "exclude")
  # Edge properties
  ...
  if(length(title) ==1)p1 <- p1 + ggtitle(title[1]) else p1 <- p1 + ggtitle(title[1], title[2])
  if(rename_nodes){ p1 <- p1 +
    geom_node_text(aes(label = new_node_names_df$new_names),
                   fontface = "bold",
                   nudge_y = 0.05)
  }else{
                     p1<- p1+
